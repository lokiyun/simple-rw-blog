...
describe('categories', () => {
  ...
  scenario('creates a category', async () => {
    const result = await createCategory({
      input: { name: 'String9831829', updatedAt: '2022-05-26T05:56:19Z' },
    })

    expect(result.name).toEqual('String9831829')
    expect(result.updatedAt).toEqual('2022-05-26T05:56:19Z')
  })
  ...
  scenario('updates a category', async (scenario: StandardScenario) => {
    ...
    const result = await updateCategory({
      id: original.id,
      input: { name: 'String65590112' },
    })

    expect(result.name).toEqual('String65590112')
  })
